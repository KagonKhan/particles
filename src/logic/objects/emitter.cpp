...
void Emitter::spawn(ParticlePool& pool, float dt)
{
    ...
    spawnAccumulator_ -= to_spawn;

    constexpr float JITTER = 0.1F;

    for (int i = 0; i < to_spawn; ++i) {
        glm::vec2 jitter {rng_.range(-JITTER, JITTER), rng_.range(-JITTER, JITTER)};
...
void Emitter::renderSettings(ParticlePool& pool)
{
    ...
    static std::size_t min_particles = 100;
    static std::size_t max_particles = 2'000'000;
